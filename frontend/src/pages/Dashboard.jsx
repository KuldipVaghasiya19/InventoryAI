...
const Dashboard = () => {
  ...
  const handleDataUpload = async (file) => {
    ...
    // 👉 Call backend to get allowed date range
    const formData = new FormData();
    formData.append("train_file", file);

    try {
      const res = await fetch("http://127.0.0.1:8000/get_date_range", {
        method: "POST",
        body: formData,
      });
      ...
      console.log("Allowed Date Range:", data);

      // ✅ Set allowed start & end months in state
      setStartMonth(data.allowed_start_month);
      setEndMonth(data.allowed_end_month);

      // Optionally, you can disable manual change by commenting out inputs later
    } catch (err) {
      console.error("Error fetching allowed range:", err);
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center">
        ...
        >
          Upload your inventory data and let our AI analyze trends, predict
          demand, and optimize your stock levels.
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <button
          ...
          className={`w-full mt-4 px-6 py-3 rounded-lg font-medium transition-all duration-200 ${
            !uploadedFile || !startMonth || !endMonth || isGenerating
      ...
      {uploadedData && (
        ...
        >
          ...
          >
            ✅ Successfully loaded <strong>{fileName}</strong> with{" "}
